...
class Members(IncrementalMailChimpStream):
    cursor_field = "timestamp"
    filter_field = "since"
    sort_field = "create_time"
    data_field = "emails"
    primary_key = ["timestamp", "id", "action"]
    ...
    def stream_slices(
        ...
        if self.id:
            ...
            lsts = Lists(authenticator=self.authenticator).read_records(sync_mode=SyncMode.full_refresh)
        for lst in lsts:
            slice_ = {"id": lst["id"]}
            cursor_value = stream_state.get(lst["id"], {}).get(self.cursor_field)
            if cursor_value:
                slice_[self.filter_field] = cursor_value
            yield slice_
    ...
    def path(self, stream_slice: Mapping[str, Any] = None, **kwargs) -> str:
        id = stream_slice["id"]
        return f"reports/{id}/email-activity"

    def get_updated_state(self, current_stream_state: MutableMapping[str, Any], latest_record: Mapping[str, Any]) -> Mapping[str, Any]:
        id = latest_record.get("id")
        latest_cursor_value = latest_record.get(self.cursor_field)
        current_stream_state = current_stream_state or {}
        current_state = current_stream_state.get(id) if current_stream_state else None
        if current_state:
            current_state = current_state.get(self.cursor_field)
        current_state_value = current_state or latest_cursor_value
        max_value = max(current_state_value, latest_cursor_value)
        ...
        current_stream_state[id] = new_value
        return current_stream_state
